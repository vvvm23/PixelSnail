...
with torch.no_grad():
    sample = torch.zeros(NB_SAMPLES, *IMAGE_DIM, dtype=torch.int64).to(device)
    pb = tqdm(total=IMAGE_DIM[0]*IMAGE_DIM[1])

    cache = {}
    for i in range(IMAGE_DIM[0]):
        for j in range(IMAGE_DIM[1]):
            pred, cache = model(sample, cache=cache)
            pred = pred.to(device)
    ...
    torchvision.utils.save_image(sample.unsqueeze(1) / 255., f"samples/zero-{save_id}.png", nrow=10)
